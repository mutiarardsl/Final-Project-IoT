...
// Threshold values - DISESUAIKAN UNTUK DEBUGGING
#define LIGHT_THRESHOLD 30       // Cahaya < 30% = gelap
#define TEMP_THRESHOLD 25.0      
#define DISTANCE_THRESHOLD 20.0  

// PIR Debounce - PENTING untuk menghindari false trigger
#define PIR_DEBOUNCE_TIME 3000   // 3 detik cooldown setelah motion
unsigned long lastMotionTime = 0;
bool motionCooldown = false;

// LDR CALIBRATION - SESUAIKAN DENGAN SENSOR ANDA
#define LDR_INVERT true  // Set true jika nilai tinggi = gelap
...
int lightLevel = 0;
int lightLevelRaw = 0;  // Untuk debugging
...
const char* TOPIC_MODE = "Klp2/esp32/mode/control";
...
void printBanner() {
  ...
  Serial.println("║     Fixed Version - Debugging Enabled          ║");
  Serial.println("╚═════════════════════════════════════════════════╝\n");
}
...
void calibrateLDR() {
  ...
  // Baca LDR beberapa kali untuk lihat range
  int sum = 0;
  for (int i = 0; i < 10; i++) {
  ...
  if (avgRaw < 500) {
    Serial.println("   ⚠️  WARNING: LDR reads very LOW (might be covered or dark)");
  } else if (avgRaw > 3500) {
    Serial.println("   ⚠️  WARNING: LDR reads very HIGH (might be very bright)");
  } else {
    Serial.println("   ✓ LDR reading normal");
  }
  
  Serial.println("   TIP: Cover LDR to see LOW values, expose to light for HIGH");
  Serial.println();
}
...
void initializeSensors() {
  ...
  if (HAS_PIR) {
    ...
    Serial.println("  ✓ PIR Sensor (with 3s debounce)");
  }
}
...
float readDistance() {
  ...
  if (duration == 0) return 999;  // Return high value jika no echo
...
int readLightLevel() {
  ...
  // MAPPING YANG BENAR - SESUAIKAN DENGAN SENSOR ANDA
  // Test dulu: tutup LDR, lihat nilai raw naik/turun?
  
  if (LDR_INVERT) {
    // Jika nilai tinggi = gelap (LDR ke ground)
    return map(lightLevelRaw, 0, 4095, 100, 0);
  } else {
    // Jika nilai tinggi = terang (LDR ke VCC) - COMMON
...
bool readMotion() {
  ...
  // Debounce logic - ignore rapid changes
  if (currentState && !motionPrevious) {
    // Rising edge - motion baru terdeteksi
    if (!motionCooldown) {
      motionPrevious = true;
    ...
    // Falling edge - motion hilang
    motionPrevious = false;
  }
  
  // Reset cooldown setelah waktu tertentu
...
void runAutomation() {
  ...
  // SCENARIO 2: Security Alarm - HANYA saat motion detected
  if (AUTO_SECURITY && HAS_PIR) {
    if (motionDetected) {
      buzzerAuto = true;
      ledAuto = true;  // Override LED untuk security
    }
  }
  ...
  // SCENARIO 4: Proximity Alert - HANYA saat jarak dekat
  if (AUTO_PROXIMITY && HAS_ULTRASONIC) {
    if (distance > 0 && distance < DISTANCE_THRESHOLD) {
      // Jangan override buzzer jika security alarm aktif
      if (!buzzerAuto) {
        buzzerAuto = true;
      }
    }
...
void publishSensorData() {
  ...
  if (HAS_PIR) {
    ...
    if (motionCooldown) {
      unsigned long remaining = PIR_DEBOUNCE_TIME - (millis() - lastMotionTime);
      Serial.printf("│    (Cooldown: %lu ms remaining)                │\n", remaining);
    }
  }
